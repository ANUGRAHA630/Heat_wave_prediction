...
# Updated output folder for Dimapur
output_folder = r"C:\Users\tjpri\Downloads\dataset\maxtemp\outputs_tmax_dimapur"
os.makedirs(output_folder, exist_ok=True)

# === Dimapur coordinates ===
target_lat, target_lon = 25.91, 93.73   # Dimapur, Nagaland
...
for file in sorted(os.listdir(input_folder)):
    ...
    try:
        ...
        print(f"⚠️ No valid pixel for {year} near Dimapur")
        continue
...
if all_data:
    ...
    out_csv = os.path.join(
        output_folder, "Dimapur_Tmax_Daily_1979_2024.csv"
    )

    final.to_csv(out_csv, index=False, float_format="%.2f")
    print(f"\n🎉 Saved Dimapur Tmax (1979–2024) → {out_csv}")
else:
    print("\n⚠️ No valid data found for Dimapur in the specified period.")
