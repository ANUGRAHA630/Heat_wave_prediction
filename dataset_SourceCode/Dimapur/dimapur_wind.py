...
output_folder = "dimapur_surface_daily_csv"
...
for year in years:
    print(f"\n📅 Downloading ERA5 Surface Wind Data for {year} (Dimapur, Nagaland)...")

    output_nc_hourly = f"dimapur_surface_6hourly_{year}.nc"
    output_nc_daily = f"dimapur_surface_daily_{year}.nc"
    output_csv_daily = os.path.join(
        output_folder, f"dimapur_surface_daily_{year}.csv"
    )

    # === Dimapur region bounding box ===
    ...
    request = {
        ...
        # Bounding box: ±0.5° around Dimapur center
...
if all_daily_files:
    ...
    merged_csv_path = os.path.join(
        output_folder, "dimapur_surface_daily_1979_2024.csv"
    )
