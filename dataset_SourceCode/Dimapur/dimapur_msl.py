...
output_folder = "dimapur_surface_daily_msl_ssrd_msnlwrf_csv"
...
# === Main Loop ===
for year in years:
    print(f"\n📅 Processing year {year} for Dimapur...")
    dfs_year = []

    for short, varname in variables.items():
        filename_hourly = f"dimapur_{short}_{year}.nc"
        filename_daily = f"dimapur_{short}_daily_{year}.nc"
        csv_out = os.path.join(
            output_folder,
            f"dimapur_{short}_daily_{year}.csv"
        )

        # 🔹 Dimapur bounding box (±0.5°)
    ...
    if dfs_year:
        ...
        yearly_csv = os.path.join(
            output_folder,
            f"dimapur_surface_daily_{year}.csv"
        )
...
if all_daily_files:
    ...
    merged_csv = os.path.join(
        output_folder,
        "dimapur_surface_daily_1979_2024.csv"
    )
