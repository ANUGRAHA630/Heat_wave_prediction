...
OUTPUT_FILE = "dimapur_daily_avg_rainfall_1979_2024.csv"

# Dimapur bounding box (~25.91°N, 93.73°E)
LAT_MIN, LAT_MAX = 25.41, 26.41
LON_MIN, LON_MAX = 93.23, 94.23
...
for nc_path in nc_files:
    ...
    rain_candidates = [
        ...
        if any(key in v.lower() for key in ["rain", "rf", "precip", "pr", "tp"])
    ]
    ...
    if ds_dimapur[rain_var].size == 0:
        print("⚠️ Region slice empty for Dimapur, skipping file.")
        continue

    # --- Convert to DataFrame ---
    df = ds_dimapur[[rain_var]].to_dataframe().reset_index()
...
# === MERGE ALL YEARS ===
if not all_data:
    raise RuntimeError("❌ No valid rainfall data found for Dimapur!")

combined_df = pd.concat(all_data, ignore_index=True)
combined_df.sort_values("date", inplace=True)

# Standardize date format for the final CSV
