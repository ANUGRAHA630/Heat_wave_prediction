...
# --- Output folder for Dimapur ---
base_folder = os.path.join("era5_rh_1000hpa", "dimapur")
os.makedirs(base_folder, exist_ok=True)

all_daily_files = []

for year in years:
    print(f"\n📅 Downloading ERA5 Relative Humidity for Dimapur – {year}...")

    output_nc_hourly = os.path.join(base_folder, f"dimapur_rh_6hourly_{year}.nc")
    output_nc_daily  = os.path.join(base_folder, f"dimapur_rh_daily_{year}.nc")
    output_csv_daily = os.path.join(base_folder, f"dimapur_rh_daily_{year}.csv")
    ...
    request = {
        ...
        # Dimapur bounding box (N, W, S, E)
        # Center: ~25.91°N, 93.73°E
        "area": [26.41, 93.23, 25.41, 94.23],
    }
    ...
    try:
        ...
        # Retry with NetCDF4 engine if default fails
...
if all_daily_files:
    ...
    merged_csv = os.path.join(base_folder, "dimapur_rh_daily_1979_2024.csv")
    df_all.to_csv(merged_csv, index=False, float_format="%.4f")

    print("\n🎉 All years processed successfully for Dimapur!")
