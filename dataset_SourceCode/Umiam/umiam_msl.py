...
output_folder = "umiam_surface_daily_msl_ssrd_msnlwrf_csv"
...
for year in years:
    ...
    for short, varname in variables.items():
        filename_hourly = f"umiam_{short}_{year}.nc"
        filename_daily = f"umiam_{short}_daily_{year}.nc"
        csv_out = os.path.join(
            output_folder,
            f"umiam_{short}_daily_{year}.csv"
        )

        # 🔹 Umiam bounding box (±0.5°)
    ...
    if dfs_year:
        ...
        yearly_csv = os.path.join(
            output_folder,
            f"umiam_surface_daily_{year}.csv"
        )
...
merged_csv = os.path.join(
    output_folder,
    "umiam_surface_daily_1979_2024.csv"
)
