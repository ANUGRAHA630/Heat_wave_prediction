...
OUTPUT_FILE = "umiam_daily_avg_rainfall_1979_2024.csv"

# Umiam bounding box (~25.66°N, 91.90°E)
LAT_MIN, LAT_MAX = 25.16, 26.16
LON_MIN, LON_MAX = 91.40, 92.40
...
for nc_path in nc_files:
    ...
    # --- Subset Umiam region ---
    # Note: If slice is empty, some datasets use descending latitudes; 
    # if so, swap LAT_MIN and LAT_MAX in the slice.
    ds_umiam = ds.sel(
        {lat_name: slice(LAT_MIN, LAT_MAX),
         lon_name: slice(LON_MIN, LON_MAX)}
    )

    if ds_umiam[rain_var].size == 0:
        ds_umiam = ds.sel(
    ...
    df = ds_umiam[[rain_var]].to_dataframe().reset_index()
...
# === MERGE ALL YEARS ===
if not all_data:
    raise RuntimeError("❌ No valid rainfall data found for Umiam!")
