...
# === Folders ===
# Keeping your original input path structure
input_folder = r"C:\Users\tjpri\Downloads\dataset\maxtemp\tmax_data\maxtemp"
# Updated output folder for Umiam
output_folder = r"C:\Users\tjpri\Downloads\dataset\maxtemp\outputs_tmax_umiam"
os.makedirs(output_folder, exist_ok=True)

# === Umiam coordinates ===
target_lat, target_lon = 25.66, 91.90   # Umiam, Meghalaya
...
def find_nearest_valid_pixel(data, lats, lons, lat, lon):
    ...
    for radius in range(0, 6):
        ...
        if np.any(valid_mask):
            ...
            # Adjusting index to the subgrid position
...
for file in sorted(os.listdir(input_folder)):
    ...
    print(f"\nProcessing year {year} for Umiam...")
    ...
    try:
        ...
        print(f"⚠️ No valid pixel for {year} near Umiam")
        continue
...
if all_data:
    ...
    out_csv = os.path.join(
        output_folder, "Umiam_Tmax_Daily_1979_2024.csv"
    )

    final.to_csv(out_csv, index=False, float_format="%.2f")
    print(f"\n🎉 Saved Umiam Tmax (1979–2024) → {out_csv}")
else:
    print("\n⚠️ No valid data found for Umiam in the specified period.")
