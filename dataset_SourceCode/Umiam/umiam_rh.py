...
# --- Output folder for Umiam ---
base_folder = os.path.join("era5_rh_1000hpa", "umiam")
os.makedirs(base_folder, exist_ok=True)

all_daily_files = []

for year in years:
    print(f"\n📅 Downloading ERA5 Relative Humidity for Umiam – {year}...")

    output_nc_hourly = os.path.join(base_folder, f"umiam_rh_6hourly_{year}.nc")
    output_nc_daily  = os.path.join(base_folder, f"umiam_rh_daily_{year}.nc")
    output_csv_daily = os.path.join(base_folder, f"umiam_rh_daily_{year}.csv")
    ...
    try:
        ...
        # Fallback for some environments/engine issues
    ...
    if "pressure_level" in ds.dims:
        # Ensure it handles the level even if formatted as a coordinate array
    ...
    # ERA5 RH variable is usually named 'r'
    if "r" in df.columns:
        df = df[["time", "r"]]
        df.rename(columns={"r": "relative_humidity"}, inplace=True)

    # Clip RH to physical range
...
if all_daily_files:
    ...
    merged_csv = os.path.join(base_folder, "umiam_rh_daily_1979_2024.csv")
