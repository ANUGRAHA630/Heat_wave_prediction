...
output_folder = r"C:\Users\tjpri\Downloads\dataset\maxtemp\outputs_tmax_bathinda"
os.makedirs(output_folder, exist_ok=True)

# === Bathinda coordinates ===
target_lat, target_lon = 30.21, 74.95   # Bathinda, Punjab
...
def find_nearest_valid_pixel(data, lats, lons, lat, lon):
    ...
    for radius in range(0, 6):
        ...
        if np.any(valid_mask):
            ...
            # subgrid coordinates are relative, add lat_min/lon_min to get global index
...
for file in sorted(os.listdir(input_folder)):
    ...
    print(f"\nProcessing year {year} for Bathinda...")
    ...
    try:
        ...
        print(f"⚠️ No valid pixel for {year} near Bathinda")
        continue
...
if all_data:
    ...
    out_csv = os.path.join(
        output_folder, "Bathinda_Tmax_Daily_1979_2024.csv"
    )

    final.to_csv(out_csv, index=False, float_format="%.2f")
    print(f"\n🎉 Saved Bathinda Tmax (1979–2024) → {out_csv}")
else:
    print("\n⚠️ No valid data found for Bathinda in the specified period.")
