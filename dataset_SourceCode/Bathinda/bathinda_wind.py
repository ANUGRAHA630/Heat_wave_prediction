...
# Updated output folder for Bathinda
output_folder = "bathinda_surface_daily_csv"
...
for year in years:
    print(f"\n📅 Downloading ERA5 Surface Wind Data for {year} (Bathinda, Punjab)...")

    output_nc_hourly = f"bathinda_surface_6hourly_{year}.nc"
    output_nc_daily = f"bathinda_surface_daily_{year}.nc"
    output_csv_daily = os.path.join(
        output_folder, f"bathinda_surface_daily_{year}.csv"
    )

    # === Bathinda region bounding box ===
    ...
    request = {
        ...
        # bounding box: ±0.5 degrees around city center
...
merged_csv_path = os.path.join(
    output_folder, "bathinda_surface_daily_1979_2024.csv"
)
