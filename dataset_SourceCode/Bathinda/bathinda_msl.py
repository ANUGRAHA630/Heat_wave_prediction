...
output_folder = "bathinda_surface_daily_msl_ssrd_msnlwrf_csv"
...
for year in years:
    ...
    for short, varname in variables.items():
        filename_hourly = f"bathinda_{short}_{year}.nc"
        filename_daily = f"bathinda_{short}_daily_{year}.nc"
        csv_out = os.path.join(
            output_folder,
            f"bathinda_{short}_daily_{year}.csv"
        )

        # 🔹 Bathinda bounding box (±0.5°)
    ...
    if dfs_year:
        ...
        yearly_csv = os.path.join(
            output_folder,
            f"bathinda_surface_daily_{year}.csv"
        )
...
if all_daily_files:
    ...
    merged_csv = os.path.join(
        output_folder,
        "bathinda_surface_daily_1979_2024.csv"
    )
