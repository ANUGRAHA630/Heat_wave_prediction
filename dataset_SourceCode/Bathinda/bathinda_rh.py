...
# --- Output folder for Bathinda ---
base_folder = os.path.join("era5_rh_1000hpa", "bathinda")
os.makedirs(base_folder, exist_ok=True)

all_daily_files = []

for year in years:
    print(f"\n📅 Downloading ERA5 Relative Humidity for Bathinda – {year}...")

    output_nc_hourly = os.path.join(base_folder, f"bathinda_rh_6hourly_{year}.nc")
    output_nc_daily  = os.path.join(base_folder, f"bathinda_rh_daily_{year}.nc")
    output_csv_daily = os.path.join(base_folder, f"bathinda_rh_daily_{year}.csv")
    ...
    request = {
        ...
        # Bathinda bounding box (N, W, S, E)
        # Center: ~30.21°N, 74.95°E
        "area": [30.71, 74.45, 29.71, 75.45],
    }
...
if all_daily_files:
    ...
    merged_csv = os.path.join(base_folder, "bathinda_rh_daily_1979_2024.csv")
    df_all.to_csv(merged_csv, index=False, float_format="%.4f")

    print("\n🎉 All years processed successfully for Bathinda!")
