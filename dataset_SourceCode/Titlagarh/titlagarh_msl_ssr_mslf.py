...
# Output folder for Titlagarh
output_folder = "titlagarh_surface_daily_msl_ssr_mslf_csv"
...
for year in years:
    print(f"\n📅 Processing year {year} for Titlagarh...")
    dfs_year = []

    for short, varname in variables.items():
        filename_hourly = f"{short}_{year}.nc"
        filename_daily = f"{short}_daily_{year}.nc"
        csv_out = os.path.join(output_folder, f"{short}_daily_{year}.csv")

        # 🔹 Titlagarh area: [N, W, S, E]
        request = {
            ...
            "area": [20.8, 82.6, 19.8, 83.6],  # 🔸 Titlagarh bounding box
        }
        ...
        for attempt in range(3):
            try:
                ...
                if wait_for_file(filename_hourly):
                    ...
                    print(f"⚠️ File incomplete, retrying...")
    ...
    if dfs_year:
        ...
        yearly_csv = os.path.join(output_folder, f"titlagarh_surface_daily_{year}.csv")
...
merged_csv = os.path.join(output_folder, "titlagarh_surface_daily_1979_2025.csv")
