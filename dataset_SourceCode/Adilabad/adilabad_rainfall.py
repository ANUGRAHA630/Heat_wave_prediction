...
# === USER SETTINGS ===
# Use the directory where your IMD rainfall NetCDF files are stored
DATA_FOLDER = r"C:\Users\tjpri\Downloads\dataset\rainfall\rainfall\rainfall"
OUTPUT_FILE = "adilabad_daily_avg_rainfall_1979_2024.csv"

# Adilabad bounding box (Center: ~19.67°N, 78.53°E)
# Format: North, West, South, East (±0.5 degrees around center)
LAT_MIN, LAT_MAX = 19.17, 20.17
LON_MIN, LON_MAX = 78.03, 79.03
...
for nc_path in nc_files:
    ...
    # --- Detect LAT / LON / TIME ---
    # Automatically find coordinate names (e.g., 'lat', 'latitude', 'lon', 'longitude')
    ...
    # --- Subset Adilabad region ---
    # slice(MIN, MAX) works if coordinates are ascending
    ds_adilabad = ds.sel(
        {lat_name: slice(LAT_MIN, LAT_MAX),
         lon_name: slice(LON_MIN, LON_MAX)}
    )

    if ds_adilabad[rain_var].size == 0:
        print("⚠️ Region slice empty, skipping file.")
        continue

    # --- Convert to DataFrame ---
    df = ds_adilabad[[rain_var]].to_dataframe().reset_index()
    ...
    # Groups by date to handle spatial mean across the selected grid pixels
    daily_avg = df.groupby("date")["rainfall_mm"].mean().reset_index()
    all_data.append(daily_avg)

# === MERGE ALL YEARS ===
if not all_data:
    raise RuntimeError("❌ No valid rainfall data found for Adilabad!")

combined_df = pd.concat(all_data, ignore_index=True)
combined_df.sort_values("date", inplace=True)

# Save to the CSV file defined in settings
