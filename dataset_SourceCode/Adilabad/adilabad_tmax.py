...
output_folder = r"C:\Users\tjpri\Downloads\dataset\maxtemp\outputs_tmax_adilabad"
os.makedirs(output_folder, exist_ok=True)

# === Adilabad coordinates ===
target_lat, target_lon = 19.66, 78.53   # Adilabad, Telangana
...
def find_nearest_valid_pixel(data, lats, lons, lat, lon):
    ...
    for radius in range(0, 6):
        ...
        if np.any(valid_mask):
            ...
            # valid_pos[1] is lat offset, valid_pos[2] is lon offset from the subgrid
...
for file in sorted(os.listdir(input_folder)):
    ...
    try:
        ...
        print(f"⚠️ No valid pixel for {year} near Adilabad")
        continue
...
if all_data:
    ...
    # Final CSV path updated for Adilabad
    out_csv = os.path.join(
        output_folder, "Adilabad_Tmax_Daily_1979_2024.csv"
    )

    final.to_csv(out_csv, index=False, float_format="%.2f")
    print(f"\n🎉 Saved Adilabad Tmax (1979–2024) → {out_csv}")
else:
    print("\n⚠️ No valid data found for Adilabad in the files provided.")
