...
# Output folder
output_folder = "sirsa_surface_daily_msl_ssrd_msnlwrf_csv"
...
# === Main Loop ===
for year in years:
    print(f"\n📅 Processing year {year} for Sirsa...")
    dfs_year = []

    for short, varname in variables.items():
        filename_hourly = f"sirsa_{short}_{year}.nc"
        filename_daily = f"sirsa_{short}_daily_{year}.nc"
        csv_out = os.path.join(output_folder, f"sirsa_{short}_daily_{year}.csv")

        # 🔹 Sirsa bounding box (±0.5°)
    ...
    if dfs_year:
        ...
        yearly_csv = os.path.join(
            output_folder,
            f"sirsa_surface_daily_{year}.csv"
        )
