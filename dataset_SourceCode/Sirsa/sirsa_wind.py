...
output_folder = "sirsa_surface_daily_csv"
...
for year in years:
    print(f"\n📅 Downloading ERA5 Surface Wind Data for {year} (Sirsa, Haryana)...")

    output_nc_hourly = f"sirsa_surface_6hourly_{year}.nc"
    output_nc_daily = f"sirsa_surface_daily_{year}.nc"
    output_csv_daily = os.path.join(output_folder, f"sirsa_surface_daily_{year}.csv")

    # === Sirsa region bounding box ===
    # Approx center: 29.53°N, 75.03°E
    request = {
    ...
    if "u10" in df.columns and "v10" in df.columns:
        df = df[["time", "u10", "v10"]]
        df.rename(
            columns={"u10": "u_component", "v10": "v_component"},
            inplace=True
        )
...
merged_csv_path = os.path.join(
    output_folder,
    "sirsa_surface_daily_1979_2024.csv"
)
df_all.to_csv(merged_csv_path, index=False, float_format="%.4f")

print("\n🎉 All years processed successfully for Sirsa!")
