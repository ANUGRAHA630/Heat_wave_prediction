...
OUTPUT_FILE = "aizawl_daily_avg_rainfall_1979_2024.csv"

# Aizawl bounding box (~23.73°N, 92.72°E)
LAT_MIN, LAT_MAX = 23.23, 24.23
LON_MIN, LON_MAX = 92.22, 93.22
...
for nc_path in nc_files:
    ...
    if not (lat_names and lon_names and time_names):
        print(f"⚠️ Missing coordinate dimensions in {nc_path}, skipping.")
        continue
    ...
    # --- Subset Aizawl region ---
    # We check both slice directions as some NC files use descending latitudes
    ds_aizawl = ds.sel({
        lat_name: slice(LAT_MIN, LAT_MAX),
        lon_name: slice(LON_MIN, LON_MAX)
    })

    if ds_aizawl[rain_var].size == 0:
        ds_aizawl = ds.sel({
    ...
    if ds_aizawl[rain_var].size == 0:
        print("⚠️ Region slice empty for Aizawl, skipping file.")
        continue

    # --- Convert to DataFrame ---
    df = ds_aizawl[[rain_var]].to_dataframe().reset_index()
...
# === MERGE ALL YEARS ===
if not all_data:
    raise RuntimeError("❌ No valid rainfall data found for Aizawl!")
