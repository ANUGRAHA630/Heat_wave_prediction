...
output_folder = "aizawl_surface_daily_msl_ssrd_msnlwrf_csv"
...
for year in years:
    ...
    for short, varname in variables.items():
        filename_hourly = f"aizawl_{short}_{year}.nc"
        filename_daily = f"aizawl_{short}_daily_{year}.nc"
        csv_out = os.path.join(
            output_folder,
            f"aizawl_{short}_daily_{year}.csv"
        )

        # 🔹 Aizawl bounding box (±0.5°)
    ...
    if dfs_year:
        ...
        yearly_csv = os.path.join(
            output_folder,
            f"aizawl_surface_daily_{year}.csv"
        )
...
merged_csv = os.path.join(
    output_folder,
    "aizawl_surface_daily_1979_2024.csv"
)
