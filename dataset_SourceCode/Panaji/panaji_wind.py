...
client = cdsapi.Client(
    ...
    key='18f04497-8219-49a6-aefe-4826e9907662'  # your CDS API key
)
...
output_folder = "panaji_surface_daily_csv"
...
for year in years:
    print(f"\n📅 Downloading ERA5 Surface Wind Data for {year} (Panaji, Goa)...")

    output_nc_hourly = f"panaji_surface_6hourly_{year}.nc"
    output_nc_daily = f"panaji_surface_daily_{year}.nc"
    output_csv_daily = os.path.join(output_folder, f"panaji_surface_daily_{year}.csv")

    # === Panaji region bounding box ===
    # Approx center: 15.49°N, 73.83°E
    request = {
...
merged_csv_path = os.path.join(output_folder, "panaji_surface_daily_1979_2024.csv")
df_all.to_csv(merged_csv_path, index=False, float_format="%.4f")

print("\n🎉 All years processed successfully for Panaji!")
