...
# Output folder
output_folder = "panaji_surface_daily_msl_ssrd_msnlwrf_csv"
...
# === Main Loop ===
for year in years:
    print(f"\n📅 Processing year {year} for Panaji (Goa)...")
    dfs_year = []

    for short, varname in variables.items():
        filename_hourly = f"panaji_{short}_{year}.nc"
        filename_daily = f"panaji_{short}_daily_{year}.nc"
        csv_out = os.path.join(output_folder, f"panaji_{short}_daily_{year}.csv")

        # 🔹 Panaji bounding box (±0.5°)
    ...
    if dfs_year:
        ...
        yearly_csv = os.path.join(
            output_folder,
            f"panaji_surface_daily_{year}.csv"
        )
...
merged_csv = os.path.join(
    output_folder,
    "panaji_surface_daily_1979_2024.csv"
)
