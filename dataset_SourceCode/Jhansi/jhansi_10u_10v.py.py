...
output_folder = r"D:\jhansi_surface_wind"
...
for year in years:
    print(f"\n📅 Wind Data {year} – Jhansi")

    nc_file = os.path.join(output_folder, f"jhansi_wind_6hr_{year}.nc")
    csv_out = os.path.join(output_folder, f"jhansi_wind_daily_{year}.csv")
...
pd.concat([pd.read_csv(f) for f in all_csvs]).to_csv(
    os.path.join(output_folder, "jhansi_wind_1979_2024.csv"),
    index=False
)

print("✅ Jhansi wind data completed")
