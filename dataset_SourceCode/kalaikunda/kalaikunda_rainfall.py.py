...
BASE = r"C:\project\dataset\Kalaikunda"
...
for file in os.listdir(input_folder):
    ...
    out = os.path.join(output_folder, f"kalaikunda_rain_{extract_year(file)}.csv")
    df.to_csv(out, index=False)
    all_csvs.append(out)

pd.concat([pd.read_csv(f) for f in all_csvs]).to_csv(
    os.path.join(output_folder, "kalaikunda_rain_1979_2024.csv"),
    index=False
)

print("✅ Kalaikunda rainfall completed")
