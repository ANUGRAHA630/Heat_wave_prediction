...
# --- Define range of years (1979–2024 inclusive) ---
...
# --- Nested Output folder: era5_rh_1000hpa/sri_ganganagar ---
base_folder = os.path.join("era5_rh_1000hpa", "sri_ganganagar")
...
for year in years:
    print(f"\n📅 Downloading ERA5 Relative Humidity for Sri Ganganagar - {year}...")
    output_nc_hourly = os.path.join(base_folder, f"sri_ganganagar_rh_6hourly_{year}.nc")
    output_nc_daily  = os.path.join(base_folder, f"sri_ganganagar_rh_daily_{year}.nc")
    output_csv_daily = os.path.join(base_folder, f"sri_ganganagar_rh_daily_{year}.csv")
    ...
    request = {
        ...
        # 🔸 Sri Ganganagar region (North, West, South, East)
        "area": [30.4, 73.4, 29.4, 74.4],
    }
    ...
    if "r" in df.columns:
        ...
        rh_col = next((c for c in df.columns if "rel" in c.lower() and "hum" in c.lower()), None)
        if rh_col:
            df = df[["time", rh_col]].rename(columns={rh_col: "relative_humidity"})
        else:
            # if still not found, keep all and warn
            print("⚠️ Could not detect RH column explicitly; keeping all columns.")
...
if all_daily_files:
    ...
    df_all = df_all.dropna(subset=["time"]).sort_values("time").reset_index(drop=True)

    merged_csv = os.path.join(base_folder, "sri_ganganagar_rh_daily_1979_2024.csv")
    # safe overwrite
    try:
        if os.path.exists(merged_csv):
            os.remove(merged_csv)
    except PermissionError:
        merged_csv = os.path.join(base_folder, "sri_ganganagar_rh_daily_1979_2024_new.csv")
    ...
    print("⚠️ No yearly CSVs to merge.")
