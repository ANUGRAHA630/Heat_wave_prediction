...
# Nested output folder: era5_wind10m/sri_ganganagar
output_folder = os.path.join("era5_wind10m", "sri_ganganagar")
...
for year in years:
    print(f"\n📅 Downloading ERA5 Surface Wind Data for Sri Ganganagar - {year}...")
    output_nc_hourly = os.path.join(output_folder, f"sri_ganganagar_surface_6hourly_{year}.nc")
    output_nc_daily  = os.path.join(output_folder, f"sri_ganganagar_surface_daily_{year}.nc")
    output_csv_daily = os.path.join(output_folder, f"sri_ganganagar_surface_daily_{year}.csv")
    ...
    request = {
        ...
        # Sri Ganganagar bounding box [North, West, South, East]
    ...
    try:
        ...
        if not wait_for_file(output_nc_hourly):
            print(f"❌ File {output_nc_hourly} is still locked after waiting. Skipping year {year}.")
            continue
    ...
    # --- Step 3: Rename 'valid_time' → 'time' if present ---
    if "valid_time" in ds.coords:
        ds = ds.rename({"valid_time": "time"})

    # --- Step 4: Average over spatial & ensemble dims ---
...
merged_csv_path = os.path.join(output_folder, "sri_ganganagar_surface_daily_1979_2024.csv")
