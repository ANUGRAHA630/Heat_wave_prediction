...
# Make sure your API key is configured in ~/.cdsapirc or provided here
...
output_folder = "gangtok_surface_daily_csv"
...
for year in years:
    print(f"\n📅 Downloading ERA5 Surface Wind Data for {year} (Gangtok, Sikkim)...")

    output_nc_hourly = f"gangtok_surface_6hourly_{year}.nc"
    output_nc_daily = f"gangtok_surface_daily_{year}.nc"
    output_csv_daily = os.path.join(
        output_folder, f"gangtok_surface_daily_{year}.csv"
    )

    # === Gangtok region bounding box ===
    # Center: ~27.33°N, 88.62°E
    ...
    request = {
        ...
        # Bounding box: ±0.5 degrees around Gangtok
        "area": [27.83, 88.12, 26.83, 89.12],
    }
    ...
    # Handle ERA5 specific ensemble/version dims if present
    ...
    # Column mapping (ERA5 defaults are u10 and v10)
    ...
    # Clean up large NetCDF files to save storage
    if os.path.exists(output_nc_hourly):
        os.remove(output_nc_hourly)


# --- Step 6: Merge all yearly CSVs ---
if all_daily_files:
    print("\n🔗 Combining all yearly CSVs into final master file...")
    ...
    merged_csv_path = os.path.join(
        output_folder, "gangtok_surface_daily_1979_2024.csv"
    )

    df_all.to_csv(merged_csv_path, index=False, float_format="%.4f")

    print(f"\n🎉 All years processed successfully for Gangtok!")
