...
# === Folders ===
# Keep your original input path structure
input_folder = r"C:\Users\tjpri\Downloads\dataset\maxtemp\tmax_data\maxtemp"
# Updated output folder for Gangtok
output_folder = r"C:\Users\tjpri\Downloads\dataset\maxtemp\outputs_tmax_gangtok"
os.makedirs(output_folder, exist_ok=True)

# === Gangtok coordinates ===
target_lat, target_lon = 27.33, 88.62   # Gangtok, Sikkim
...
def find_nearest_valid_pixel(data, lats, lons, lat, lon):
    ...
    for radius in range(0, 6):
        ...
        if np.any(valid_mask):
            ...
            # Adjusting index to the global grid position
...
for file in sorted(os.listdir(input_folder)):
    ...
    try:
        ...
        print(f"⚠️ No valid pixel for {year} near Gangtok")
        continue
...
if all_data:
    ...
    out_csv = os.path.join(
        output_folder, "Gangtok_Tmax_Daily_1979_2024.csv"
    )

    final.to_csv(out_csv, index=False, float_format="%.2f")
    print(f"\n🎉 Saved Gangtok Tmax (1979–2024) → {out_csv}")
else:
    print("\n⚠️ No valid data found for Gangtok in the specified period.")
