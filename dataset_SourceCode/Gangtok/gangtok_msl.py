...
output_folder = "gangtok_surface_daily_msl_ssrd_msnlwrf_csv"
...
# === Main Loop ===
for year in years:
    print(f"\n📅 Processing year {year} for Gangtok...")
    dfs_year = []

    for short, varname in variables.items():
        filename_hourly = f"gangtok_{short}_{year}.nc"
        filename_daily = f"gangtok_{short}_daily_{year}.nc"
        csv_out = os.path.join(
            output_folder,
            f"gangtok_{short}_daily_{year}.csv"
        )

        # 🔹 Gangtok bounding box (±0.5°)
    ...
    if dfs_year:
        ...
        yearly_csv = os.path.join(
            output_folder,
            f"gangtok_surface_daily_{year}.csv"
        )
...
if all_daily_files:
    ...
    merged_csv = os.path.join(
        output_folder,
        "gangtok_surface_daily_1979_2024.csv"
    )
    ...
    print("\n⚠️ No data files found to merge.")
