...
output_folder = "rajkot_surface_daily_csv"
...
for year in years:
    print(f"\n📅 Downloading ERA5 Surface Wind Data for {year} (Rajkot, Gujarat)...")

    output_nc_hourly = f"rajkot_surface_6hourly_{year}.nc"
    output_nc_daily = f"rajkot_surface_daily_{year}.nc"
    output_csv_daily = os.path.join(output_folder, f"rajkot_surface_daily_{year}.csv")

    # === Rajkot region bounding box ===
    # Approx center: 22.3039°N, 70.8022°E
    request = {
...
merged_csv_path = os.path.join(output_folder, "rajkot_surface_daily_1979_2024.csv")
