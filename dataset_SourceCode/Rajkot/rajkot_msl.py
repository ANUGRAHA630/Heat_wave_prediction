...
# Output folder
output_folder = "rajkot_surface_daily_msl_ssrd_msnlwrf_csv"
...
# === Main Loop ===
for year in years:
    print(f"\n📅 Processing year {year} for Rajkot...")
    dfs_year = []

    for short, varname in variables.items():
        filename_hourly = f"rajkot_{short}_{year}.nc"
        filename_daily = f"rajkot_{short}_daily_{year}.nc"
        csv_out = os.path.join(output_folder, f"rajkot_{short}_daily_{year}.csv")

        # 🔹 Rajkot bounding box (±0.5°)
        # Center: 22.3039°N, 70.8022°E
        request = {
    ...
    # --- Step 6: Merge variables per year ---
    if dfs_year:
        merged = dfs_year[0]
        for df in dfs_year[1:]:
            merged = pd.merge(merged, df, on="time", how="outer")

        yearly_csv = os.path.join(output_folder, f"rajkot_surface_daily_{year}.csv")
...
merged_csv = os.path.join(
    output_folder,
    "rajkot_surface_daily_1979_2024.csv"
)
df_all.to_csv(merged_csv, index=False, float_format="%.4f")

print("\n🎉 All years processed successfully for Rajkot!")
