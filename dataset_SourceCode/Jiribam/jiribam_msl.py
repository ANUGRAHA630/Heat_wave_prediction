...
output_folder = "jiribam_surface_daily_msl_ssrd_msnlwrf_csv"
...
for year in years:
    ...
    for short, varname in variables.items():
        filename_hourly = f"jiribam_{short}_{year}.nc"
        filename_daily = f"jiribam_{short}_daily_{year}.nc"
        csv_out = os.path.join(
            output_folder,
            f"jiribam_{short}_daily_{year}.csv"
        )

        # 🔹 Jiribam bounding box (±0.5°)
    ...
    if dfs_year:
        ...
        yearly_csv = os.path.join(
            output_folder,
            f"jiribam_surface_daily_{year}.csv"
        )
...
merged_csv = os.path.join(
    output_folder,
    "jiribam_surface_daily_1979_2024.csv"
)
