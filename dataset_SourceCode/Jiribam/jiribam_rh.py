...
# --- Output folder for Jiribam ---
base_folder = os.path.join("era5_rh_1000hpa", "jiribam")
os.makedirs(base_folder, exist_ok=True)

all_daily_files = []

for year in years:
    print(f"\n📅 Downloading ERA5 Relative Humidity for Jiribam – {year}...")

    output_nc_hourly = os.path.join(base_folder, f"jiribam_rh_6hourly_{year}.nc")
    output_nc_daily  = os.path.join(base_folder, f"jiribam_rh_daily_{year}.nc")
    output_csv_daily = os.path.join(base_folder, f"jiribam_rh_daily_{year}.csv")
    ...
    request = {
        ...
        # Jiribam bounding box (N, W, S, E)
        # Center: ~24.80°N, 93.12°E
        "area": [25.30, 92.62, 24.30, 93.62],
    }
...
if all_daily_files:
    ...
    merged_csv = os.path.join(base_folder, "jiribam_rh_daily_1979_2024.csv")
    df_all.to_csv(merged_csv, index=False, float_format="%.4f")

    print("\n🎉 All years processed successfully for Jiribam!")
