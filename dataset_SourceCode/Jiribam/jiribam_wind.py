...
output_folder = "jiribam_surface_daily_csv"
...
for year in years:
    print(f"\n📅 Downloading ERA5 Surface Wind Data for {year} (Jiribam, Manipur)...")

    output_nc_hourly = f"jiribam_surface_6hourly_{year}.nc"
    output_nc_daily = f"jiribam_surface_daily_{year}.nc"
    output_csv_daily = os.path.join(
        output_folder, f"jiribam_surface_daily_{year}.csv"
    )

    # === Jiribam region bounding box ===
...
merged_csv_path = os.path.join(
    output_folder, "jiribam_surface_daily_1979_2024.csv"
)
