...
output_folder = r"D:\agartala_surface_flux"
...
for year in years:
    ...
    for short, var in variables.items():
        nc = os.path.join(output_folder, f"agartala_{short}_{year}.nc")
    ...
    csv = os.path.join(output_folder, f"agartala_surface_{year}.csv")
    merged.to_csv(csv, index=False)
    all_csvs.append(csv)

pd.concat([pd.read_csv(f) for f in all_csvs]).to_csv(
    os.path.join(output_folder, "agartala_surface_1979_2024.csv"),
    index=False
)

print("✅ Agartala surface variables completed")
