...
output_folder = r"D:\dehradun_surface_wind"
...
for year in years:
    print(f"\n📅 Wind Data {year} – Dehradun")

    nc_file = os.path.join(output_folder, f"dehradun_wind_6hr_{year}.nc")
    csv_out = os.path.join(output_folder, f"dehradun_wind_daily_{year}.csv")
...
pd.concat([pd.read_csv(f) for f in all_csvs]).to_csv(
    os.path.join(output_folder, "dehradun_wind_1979_2024.csv"),
    index=False
)

print("✅ Dehradun wind data completed")
